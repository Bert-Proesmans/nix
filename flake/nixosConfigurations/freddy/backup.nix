...
in
{
  ...
  services.sanoid =
    let
      default-settings = {
        ...
        # NOTE; For replication purposes all datasets require at least 1 snapshot for all types up to yearly, unless
        # syncoid argument --no-stream is used.
        frequently = 16; # 4 hours @ 15 minute rate
  ...
  services.syncoid = {
    ...
    commonArgs = [
      ...
      #
      # WARN; Sanoid only prunes snapshots per time-bucket and receives only frequently's. This dilutes certainty of the total timespan
      # replicated snapshots cover.
