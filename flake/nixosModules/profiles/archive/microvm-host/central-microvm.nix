{ lib, config, ... }:
{
  config = {
    systemd.mounts = lib.flatten (
      lib.flip lib.mapAttrsToList config.microvm.vms (
        name: microvm-config:
        [
          {
        ...
        ++ (lib.flip builtins.map microvm-config.config.config.microvm.central.shares (share: {
          # INFO; Mount each desired directory into the base

          what = share.source;
          where = "/run/central-microvm/${name}/${share.tag}";
          type = "none";
          options = lib.concatStringsSep "," ([ "bind" ] ++ lib.optionals share.read-only [ "ro" ]);
