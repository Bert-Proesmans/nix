...
in
{
    ...
    proxies = lib.mkOption {
      ...
      type = lib.types.listOf (
        lib.types.submodule (
          ...
          {
            options = {
              enable = lib.mkEnableOption "vsock<->tcp proxy" // {
                default = true;
              };
  ...
  config = {
    systemd.services = lib.mkMerge (
      lib.flip lib.imap1 cfg.proxies (
        i: v: {
              ...
              serviceConfig = {
                ...
                ExecStart =
                  let
                    ...
                    source-argument =
                      if v.listen.tcp.ip != null then
                        "TCP4-LISTEN:${toString v.listen.port},bind=${v.listen.tcp.ip},reuseaddr,fork"
                      else
                        "VSOCK-LISTEN:${toString v.listen.port},reuseaddr,fork";
                    destination-argument =
                      if v.transmit.tcp.ip != null then
                        "TCP4-CONNECT:${v.transmit.tcp.ip}:${toString v.transmit.port}"
                      else
                        "VSOCK-CONNECT:${toString v.transmit.vsock.cid}:${toString v.transmit.port}";
                  in
